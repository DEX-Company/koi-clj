...
(comment
  ...
  (-> "koi.examples.hashing-simple/compute-hash"
      (resolve-op)
  ...
  (-> (io/resource "hashing_asset_metadata.json")
      slurp
  ...
  ;;Example: lets use param validation interceptor to test if the keys
  ;;in input map satisfy the spec
  (let [param-spec {:to-hash {:type "asset", :position 0, :required true}}]
    (->> {:to-hash (s/memory-asset {:a :b} "content")}
         ((ki/run-chain
           ...
           sha-asset-hash))))
  ;;result
  ;;{:hash-val #object[sg.dex.starfish.impl.memory.MemoryAsset 0x37c02e35 "aa22682a710451db561b0cf52344a7e05fc9c7c46140773b3c04e71c6772a892"]}


  ;;failure case: the input spec expects an 'asset type', but the input contains a string value
  (let [param-spec {:to-hash {:type "asset", :position 0, :required true}}]
    (->> {:to-hash "abcd"}
         ((ki/run-chain
           [(ki/param-validator param-spec)]
           sha-asset-hash))))
  ;;result
  ;;{:error {:cause "mandatory params missing"}}

  ;;Similarly,validation for the result generated by the operation
  (let [param-spec {:hash-val {:type "asset", :position 0, :required true}}]
    (->> {:to-hash (s/memory-asset {:a :b} "content")}
         ((ki/run-chain
           ...
           sha-asset-hash))))
  ;;result
  ;;{:hash-val #object[sg.dex.starfish.impl.memory.MemoryAsset 0x56ddc2da "7d76c56ea751c8d4b6b95e8caf516e620bdbee4a6b8c48d167068911c8fc2d48"]}
  ...
  ;;this return a map where the values are Asset objects.
